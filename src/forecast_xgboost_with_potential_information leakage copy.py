...
if len(X_test) >= 72:
    ...
    plt.plot(range(len(future_predictions)), future_predictions, label='Predicted', color='blue', marker='o')
    plt.plot(range(len(future_predictions)), y_test.iloc[:72].values, label='Actual', color='orange', marker='x')
